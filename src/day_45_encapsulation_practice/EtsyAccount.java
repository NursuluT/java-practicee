...
public class EtsyAccount {
	...
	public EtsyAccount(String email,String firstName) {
		setEmail(email);
		setFirstName(firstName);
		this.password = getRandomPassword(); //random, password with 6 character
	}
	
	private String getRandomPassword() {
		Random rand = new Random();
		String letters ="abcdefghijklmnoprstuwxyz0123456789";
		String randPassword = "";
		for(int i =1; i<=6; i++) {
		randPassword+=letters.charAt(rand.nextInt(letters.length()));
		}
			return randPassword;
	...
	public void setEmail(String email) {
		...
		//if not show message"Please enter valid email";
	...
	public void setFirstName(String firstName) {
		//Can't be blank
		//needs to be only Alphabetic/letter
		if(isValidFirstName(firstName)) {
			this.firstName = firstName;
		}else {
			System.out.println("Your first name cannot contain invalid character");
		}
